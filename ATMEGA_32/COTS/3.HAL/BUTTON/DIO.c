	...
	switch(Cpy_u8DIO_Port)
		{
			...
			case(DIO_PORTA):
					if(Cpy_tenuPinVal==DIO_enuHigh)
						SET_BIT(GPIOA->PORT,Cpy_u8DIO_Pinnum);
					else
						CLR_BIT(GPIOA->PORT,Cpy_u8DIO_Pinnum);
					break;
			case(DIO_PORTB):
			{
				if(Cpy_tenuPinVal==DIO_enuHigh)
					SET_BIT(GPIOB->PORT,Cpy_u8DIO_Pinnum);
				else
					CLR_BIT(GPIOB->PORT,Cpy_u8DIO_Pinnum);
				
			}break;
			case(DIO_PORTC):
			{
				if(Cpy_tenuPinVal==DIO_enuHigh)
					SET_BIT(GPIOC->PORT,Cpy_u8DIO_Pinnum);
				else
					CLR_BIT(GPIOC->PORT,Cpy_u8DIO_Pinnum);
			}break;
			
			case(DIO_PORTD):
			{
				if(Cpy_tenuPinVal==DIO_enuHigh)
					SET_BIT(GPIOD->PORT,Cpy_u8DIO_Pinnum);
				else
					CLR_BIT(GPIOD->PORT,Cpy_u8DIO_Pinnum);
...
DIO_tenuErrorStatus DIO_enuGETPIN(DIO_tenuPort Cpy_u8DIO_Port,DIO_tenuPins Cpy_u8DIO_Pinnum,u8 *Ptr_Pin)
{
	...
	else
	{
		...
		{
			case(DIO_PORTA):
			*Ptr_Pin = GET_BIT(GPIOA->PIN,Cpy_u8DIO_Pinnum);
			break;
			
			case(DIO_PORTB):
			*Ptr_Pin = GET_BIT(GPIOB->PIN,Cpy_u8DIO_Pinnum);
			break;
			
			case(DIO_PORTC):
			*Ptr_Pin = GET_BIT(GPIOC->PIN,Cpy_u8DIO_Pinnum);
			break;
			
			case(DIO_PORTD):
			*Ptr_Pin = GET_BIT(GPIOD->PIN,Cpy_u8DIO_Pinnum);
			break;
